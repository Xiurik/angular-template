import { Title } from './chunk-HYD76HNN.js';
import './chunk-R3VWRG7W.js';
import {
...
function isPublicRouterEvent(e) {
  return (
    !(e instanceof BeforeActivateRoutes) && !(e instanceof RedirectRequest) && !(e instanceof BeforeRoutesRecognized)
  );
}
...
function matchWithChecks(segmentGroup, route, segments, injector, urlSerializer, createSnapshot, abortSignal) {
  ...
  return runCanMatchGuards(injector, route, segments, urlSerializer, currentSnapshot, abortSignal).pipe(
  ...
  matchSegmentAgainstRoute(injector, rawSegment, route, segments, outlet, parentRoute) {
    return __async(this, null, function* () {
      ...
      const createSnapshot = (result2) =>
        this.createSnapshot(injector, route, result2.consumedSegments, result2.parameters, parentRoute);
      const result = yield firstValueFrom(
        matchWithChecks(rawSegment, route, segments, injector, this.urlSerializer, createSnapshot, this.abortSignal)
      );
      if (route.path === '**') {
        rawSegment.children = {};
      }
...
function recognize(
  ...
  return mergeMap((t) =>
    __async(null, null, function* () {
      const { state: targetSnapshot, tree: urlAfterRedirects } = yield recognize$1(
        injector,
        configLoader,
        rootComponentType,
        config,
          ...
          switchMap((t) => {
            ...
            if (this.urlHandlingStrategy.shouldProcessUrl(t.rawUrl)) {
              return of(t).pipe(
                ...
                switchMap((value) =>
                  from(overallTransitionState.routesRecognizeHandler.deferredHandle ?? of(void 0)).pipe(
                    map(() => value)
                  )
                ),
                tap(() => {
      ...
      __async(this, null, function* () {
        ...
        this.zone.run(() => {
          this.transitions.events.next(
            new Scroll(routerEvent, this.lastSource === 'popstate' ? this.store[this.restoredId] : null, anchor, scroll)
          );
        });
      })
    );
...
var NavigationStateManager = class _NavigationStateManager extends StateManager {
  ...
  handleRouterEvent(e, transition) {
    return __async(this, null, function* () {
      ...
      if (e instanceof NavigationStart) {
        ...
        transition.routesRecognizeHandler.deferredHandle = new Promise((resolve) =>
          __async(this, null, function* () {
            if (this.urlUpdateStrategy === 'eager') {
              try {
                this.maybeCreateNavigationForTransition(transition);
        ...
        transition.beforeActivateHandler.deferredHandle = new Promise((resolve) =>
          __async(this, null, function* () {
  ...
  navigate(internalPath, transition) {
    ...
    const history =
      this.location.isCurrentPathEqualTo(path) || transition.extras.replaceUrl || transition.extras.skipLocationChange
        ? 'replace'
        : 'push';
      ...
      this.currentNavigation.commitUrl = () =>
        __async(this, null, function* () {
          ...
          if (transition && !transition.extras.skipLocationChange) {
            ...
            const history =
              this.location.isCurrentPathEqualTo(internalPath) || !!transition.extras.replaceUrl ? 'replace' : 'push';
            const state = __spreadProps(__spreadValues({}, transition.extras.state), {
              navigationId: transition.id
            });
            const pathOrUrl = this.location.prepareExternalUrl(internalPath);
